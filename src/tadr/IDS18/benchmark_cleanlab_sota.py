"""
Benchmark SOTA Cleanlab defense for IDS18.
...
NOISE_FILE_PATTERN = re.compile(r"^cic2018_train_original_undersampled_noise_(\d+)$", flags=re.IGNORECASE)
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(description="Benchmark SOTA Cleanlab defense for IDS18")
    parser.add_argument("--python-exe", type=str, default=sys.executable)
    parser.add_argument("--input-dir", type=str, default=str(script_dir / "resources" / "IDS18" / "clean_merged"))
    ...
    parser.add_argument("--sota-dir", type=str, default=str(script_dir / "reports" / "sota_cleanlab"))
    ...
    if not cleanlab_script.exists() or not training_script.exists():
        raise SystemExit("Required script not found (cleanlab_defense.py/model_training.py)")
    ...
    print("=" * 100)
    print("[DONE] SOTA CLEANLAB BENCHMARK COMPLETED (IDS18)")
