"""Train all CIC-IDS-2018 models for the ablation pipeline.

Overall flow:
1) Resolve the training CSV passed by the ablation runner.
2) Load the fixed CIC-IDS-2018 test CSV.
...
DEFAULT_TRAIN_FILE = Path(CIC2018Resources.CLEAN_MERGED_DATA_FOLDER) / "cic2018_train_original_undersampled.csv"
DEFAULT_TEST_FILE = Path(CIC2018Resources.CLEAN_MERGED_DATA_FOLDER) / "cic2018_test_original_undersampled.csv"
MODEL_ORDER = ["xgb", "catb", "bagging", "histgbm", "lgbm", "rf", "dnn"]


def _resolve_path(path_text: str | None, default: Path) -> Path:
    """Resolve a possibly relative path against the common workspace anchors."""
    if not path_text:
        return default
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(description="CIC-IDS-2018 all-model training entrypoint")
    parser.add_argument("--model", "-m", default="all", choices=["all"], help="Only 'all' is supported")
    parser.add_argument("--resource", "-r", default="cic2018", choices=["cic2018"])
    ...
    # Load the CIC-IDS-2018 preprocessor and its fitted encoders.
