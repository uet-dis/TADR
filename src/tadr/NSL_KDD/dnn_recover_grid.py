"""NSL-KDD DNN recovery pipeline for the ablation run.

Overall flow:
1) Load the filtered clean CSV and the remaining-noise CSV.
2) Load the fitted NSL-KDD encoders and encode both inputs.
...
from tadr.configs.nslkdd import NSLKDDResources
...
DEFAULT_CLEAN_INPUT = Path(NSLKDDResources.CLEAN_MERGED_DATA_FOLDER) / "nslkdd_train_clean_merged.csv"
DEFAULT_NOISE_INPUT = Path(NSLKDDResources.CLEAN_MERGED_DATA_FOLDER) / "nslkdd_train_clean_merged_noise.csv"
...
def _distribution_rows(values: pd.Series) -> list[dict]:
    """Convert label counts into JSON-friendly rows with count and rate."""
...
def encode_dataset(df: pd.DataFrame, preprocessor: NSLKDDPreprocessor) -> pd.DataFrame:
    """Apply the fixed NSL-KDD encoding pipeline to one dataframe."""
...
class RobustMLP(nn.Module):
    """Simple feed-forward classifier used as the DNN recovery model."""

    def __init__(self, input_dim: int, num_classes: int, hidden_dims: list[int], dropout: float):
        """Build a small classification network with BatchNorm and Dropout."""
        super().__init__()
    ...
    def forward(self, x):
        """Run one forward pass and return class logits."""
        return self.net(x)
...
def train_one_model(
    ...
    """Train one recovery model on the clean set with early stopping."""
    ...
    # Use class weights so rare classes still get learned during recovery.
    ...
    for epoch in range(max_epochs):
        # Training step: fit on the clean encoded samples.
        model.train()
        ...
        # Validation step: keep the model with the best clean accuracy.
        model.eval()
        val_correct = 0
        val_total = 0
        with torch.no_grad():
...
def predict_proba(model: RobustMLP, X: np.ndarray, batch_size: int, device: str) -> np.ndarray:
    """Predict softmax probabilities in batches for a large dataframe."""
    model.eval()
    probs = []
    X_tensor = torch.tensor(X, dtype=torch.float32)
...
def recovery_metrics(y_true: np.ndarray, y_pred: np.ndarray, accept_mask: np.ndarray) -> dict:
    """Compute accepted-count, precision, recall, and F1 for recovery."""
...
def log_label_distribution(name: str, df: pd.DataFrame, label_col: str = "Label") -> None:
    """Log the label distribution for quick debugging and run summaries."""
...
def per_class_recovery_stats(df_recovered: pd.DataFrame, class_names: list[str]) -> pd.DataFrame:
    """Compute per-class recovery counts and correctness rates."""
    rows = []
...
def build_accept_mask(
    ...
    """Accept only high-confidence Benign rows, then enforce optional per-class caps."""
...
def main() -> None:
    """Run the fixed NSL-KDD recovery flow used by the ablation pipeline."""
    parser = argparse.ArgumentParser(description="NSL-KDD DNN recovery for the ablation pipeline")
    parser.add_argument("--clean-in", type=str, required=True, help="Input clean CSV after the DAE/KMeans/KNN stage")
    parser.add_argument("--noise-in", type=str, required=True, help="Input remaining-noise CSV after the DAE/KMeans/KNN stage")
    ...
    # Validate input files and normalize the output directory.
    ...
    # Resolve the two output CSV paths expected by the pipeline.
    if args.output_dir is not None:
        ...
        noise_base, noise_ext = os.path.splitext(args.noise_in)
        args.output_clean = f"{clean_base}_recovered_final{clean_ext}"
        args.output_noise = f"{noise_base}_remaining_after_recover{noise_ext}"

    logger.info(f"\n{'=' * 80}")
    logger.info("NSL-KDD DNN RECOVERY PIPELINE")
    logger.info(f"{'=' * 80}")
    logger.info(f"[+] Clean input: {args.clean_in}")
    logger.info(f"[+] Noise input: {args.noise_in}")

    # Load the NSL-KDD encoder bundle once and reuse it for both inputs.
    preprocessor = NSLKDDPreprocessor()
    logger.info("[+] Loading encoders for NSL-KDD...")
    if not preprocessor.load_encoders():
        raise SystemExit("Encoders not found. Please fit/load encoders first.")

    # Read the filtered clean/noise datasets produced by the defense stage.
    ...
    # Keep metadata if it exists, but do not require it for the core path.
    ...
    # The recovery model should only see the encoded feature/label columns.
    ...
    # STEP 1: Encode the clean and noise datasets.
    logger.info(f"\n{'=' * 80}")
    ...
    # When present, use original_label for summary metrics and per-class reports.
    y_noise_true = None
    if has_original_label:
        y_noise_true = preprocessor.encoders["label"].transform(df_noise["original_label"].astype(str).values)

    # STEP 2: Train one fixed DNN recovery model.
    logger.info(f"\n{'=' * 80}")
    ...
    # Keep the DNN compact and deterministic for the ablation path.
    hidden_dims = [256, 128]
    dropout = 0.3
    lr = 5e-4
    weight_decay = 1e-4
    ...
    # STEP 3: Predict the noise set and build the recovery mask.
    logger.info(f"\n{'=' * 80}")
    ...
    # Fixed thresholds keep the acceptance rule aligned with the original pipeline intent.
    ...
    # STEP 4: Split the noise set into recovered rows and remaining noise.
    logger.info(f"\n{'=' * 80}")
    ...
    if len(df_recovered) > 0:
        # Re-label recovered rows using the DNN prediction.
    ...
    # STEP 5: Build summary reports and per-class recovery statistics.
    logger.info(f"\n{'=' * 80}")
    ...
    summary_report = {
        ...
        "clean_input_label_distribution": _distribution_rows(df_clean["Label"].astype(str)) if "Label" in df_clean.columns else [],
        "noise_input_label_distribution": _distribution_rows(df_noise["Label"].astype(str)) if "Label" in df_noise.columns else [],
    }
    ...
    if y_noise_true is not None:
        ...
        logger.info(
            f"[+] Recovery metrics -> F1={metrics['f1']:.4f}, P={metrics['precision']:.4f}, R={metrics['recall']:.4f}, accepted={metrics['accepted']}"
        )
        ...
        # Compute per-class recovery details for the final report.
            ...
            detailed_rows.append(
                {
                    ...
                    "correct_recovered_rate": float(class_correct_recovered / class_total) if class_total > 0 else 0.0,
                    "precision_on_recovered": float(class_correct_recovered / class_recovered) if class_recovered > 0 else 0.0,
        ...
        # If the input does not carry original labels, still write a valid report.
    ...
    # STEP 6: Resolve output paths and export all artifacts.
    logger.info(f"\n{'=' * 80}")
    ...
    if has_source:
        df_recovered["__source__"] = source_col.loc[df_recovered.index].values if len(df_recovered) > 0 else []
    if has_original_label:
        df_recovered["original_label"] = original_label_col.loc[df_recovered.index].values if len(df_recovered) > 0 else []
    if has_is_noisy:
        df_recovered["is_noisy"] = is_noisy_col.loc[df_recovered.index].values if len(df_recovered) > 0 else []
    ...
    logger.info(f"\n{'=' * 80}")
    logger.info("NSL-KDD DNN RECOVERY COMPLETED")
    logger.info(f"{'=' * 80}")
