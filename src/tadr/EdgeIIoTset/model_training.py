"""Train all EdgeIIoT models for the ablation pipeline.

Overall flow:
1) Resolve the training CSV passed by the caller.
2) Load the fixed EdgeIIoT test CSV.
...
from tadr.preprocessing.prepare import PrepareData
from tadr.training.bagging import BaggingModel
from tadr.training.catb import CatBoostModel
from tadr.training.dnn import DNNModel
from tadr.training.gbm import GBMModel
from tadr.training.histgbm import HistGBMModel
from tadr.training.lgbm import LGBMModel
from tadr.training.model import confusion_matrix_from_indices, evaluate_classification, save_confusion_matrix_png
from tadr.training.rf import RandomForestModel
from tadr.training.xgb import XGBModel

logger = get_logger(__name__)

RESOURCE_ROOT = SCRIPT_DIR / EDGEIOTResources.CLEAN_MERGED_DATA_FOLDER.lstrip(os.sep)
PARAMS_DIR = SCRIPT_DIR / "params" / "training"
ENCODERS_DIR = SCRIPT_DIR / EDGEIOTResources.ENCODERS_FOLDER.lstrip(os.sep)
DEFAULT_TRAIN_FILE = RESOURCE_ROOT / "edgeiot_train_clean_merged.csv"
DEFAULT_TEST_FILE = RESOURCE_ROOT / "edgeiot_test_clean_merged.csv"
DEFAULT_OUTPUT_DIR = SCRIPT_DIR / "model_training_outputs"
...
def _load_params(model_name: str) -> dict:
    ...
    raise FileNotFoundError(f"No parameter file found for {model_name}. Tried: {path}")
...
def _log_input_distribution(df: pd.DataFrame, dataset_name: str, label_column: str) -> None:
    """Log the label distribution of a dataset for quick inspection."""
    if label_column not in df.columns:
        logger.warning(f"[!] {dataset_name} does not contain '{label_column}' column. Skip distribution logging.")
        return
    ...
    dist = df[label_column].value_counts(dropna=False)
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(description="EdgeIIoT all-model training entrypoint")
    parser.add_argument("--model", "-m", default="all", choices=["all"], help="Only 'all' is supported")
    parser.add_argument("--resource", "-r", default="edgeiot", choices=["edgeiot"])
    parser.add_argument("--train-in", type=str, default=None, help="Training CSV to use for the ablation run")
    parser.add_argument("--output-dir", type=str, default=None, help="Directory where models, metrics, and CMs are written")
    ...
    pre = EDGEIOTPreprocessor()
    if not pre.load_encoders(str(ENCODERS_DIR)):
    ...
    _log_input_distribution(df_train, "Train", pre.label_column)
    _log_input_distribution(df_test, "Test", pre.label_column)
