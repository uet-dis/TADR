"""
Benchmark SOTA KNN defense for EdgeIIoTset.
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(description="Benchmark SOTA KNN defense for EdgeIIoTset")
    ...
    parser.add_argument("--only-benign", action="store_true", help="Run KNN sanitization only on current Normal rows")
    parser.add_argument("--device", type=str, default="auto", choices=["CPU", "auto"])
    parser.add_argument("--sota-dir", type=str, default=str(script_dir / "reports" / "sota-benign-only"))
    ...
    if not symmetric_script.exists() or not knn_script.exists() or not training_script.exists():
        raise SystemExit("Required script not found (symmetric_label_noise.py/knn_label_sanitization.py/model_training.py)")
    ...
    print("=" * 100)
    print("[DONE] SOTA KNN BENCHMARK COMPLETED (EDGEIOTSET)")
