...
def evaluate_classification(
    ...
    Returns a dict with accuracy, F1/precision/recall (both macro and weighted) and classes.
    """
    acc = float(accuracy_score(y_true, y_pred_idx))
    
    # Per-class F1 (aligned with class_names order: encoded indices 0..n-1)
    ...
    f1_per_class = {
        str(class_name): float(f1_per_class_arr[idx])
        for idx, class_name in enumerate(class_names)
    }

    # Macro metrics
    f1_macro = float(f1_score(y_true, y_pred_idx, average='macro'))
    precision_macro = float(precision_score(y_true, y_pred_idx, average='macro', zero_division=0))
    recall_macro = float(recall_score(y_true, y_pred_idx, average='macro', zero_division=0))
    
    # Weighted metrics
    f1_weighted = float(f1_score(y_true, y_pred_idx, average='weighted'))
    precision_weighted = float(precision_score(y_true, y_pred_idx, average='weighted', zero_division=0))
    recall_weighted = float(recall_score(y_true, y_pred_idx, average='weighted', zero_division=0))
