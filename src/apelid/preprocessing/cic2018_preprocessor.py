...
class CIC2018Preprocessor(Preprocessor):
    ...
    def _resolve_existing_encoder_dir(self, encoders_dir: str | None) -> str:
        """Resolve encoder folder from common locations and return the first existing dir."""
        base = Path(encoders_dir or self.encoders_dir)
        candidates = []
        ...
        if base.is_absolute():
            ...
            candidates.extend([
                Path.cwd() / base,
                PROJECT_ROOT / base,
                PROJECT_ROOT / "src" / "apelid" / "IDS18" / base,
                PROJECT_ROOT / "src" / "apelid" / "IDS18" / "encoders",
                PROJECT_ROOT / "src" / "apelid" / "IDS18" / "encoders" / "cic2018",
            ])
        ...
        # Keep current behavior as fallback for clearer error messages downstream.
        return str(base)
    ...
    def load_encoders(self, encoders_dir=None, fixed_label_encoder: bool = False):
        ...
        encoders_dir = self._resolve_existing_encoder_dir(encoders_dir)
