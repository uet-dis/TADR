"""
UQ-LED CL-MCD-E label-error defense for NSL-KDD.
...
from apelid.EdgeIIoTset.uqled_defense import (
    OOFMCDResult,
    UQLEDConfig,
    UQLEDMLP,
    UQLEDResult,
...
def _encode_nslkdd(df: pd.DataFrame, num_encoder: str) -> tuple[np.ndarray, np.ndarray, Any]:
    preprocessor = NSLKDDPreprocessor()
    encoders_dir = SCRIPT_DIR / "encoders" / "nslkdd"
    if not preprocessor.load_encoders(str(encoders_dir)):
        raise RuntimeError(f"NSL-KDD encoders were not found in {encoders_dir}")

    encoded = preprocessor.select_features_and_label(df).copy()
...
def main() -> None:
    parser = argparse.ArgumentParser(description="UQ-LED CL-MCD-E defense for NSL-KDD")
    ...
    X, labels, preprocessor = _encode_nslkdd(df, args.num_encoder)
    try:
