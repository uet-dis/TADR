"""
Targeted Label Flipping Generator for NSL-KDD Dataset
...
Usage:
    python symmetric_label_noise.py --input nslkdd_train_clean_merged.csv --noise-rate 0.1 --seed 42
"""
...
from pathlib import Path
    ...
    Args:
    default_input = Path(SCRIPT_DIR) / "resources" / "NSLKDD" / "clean_merged" / "nslkdd_train_clean_merged.csv"
    ...
    """
    # Core flipping routine used by the ablation pipeline. Kept minimal:
    # it flips a fraction of samples from each non-Benign class to Benign
    # and optionally annotates `original_label`/`is_noisy` for evaluation.
...
def main():
    parser = argparse.ArgumentParser(
        description="Apply targeted label flipping (non-benign -> benign) to NSL-KDD dataset"
    )
    parser.add_argument(
        '--input', '-i',
        type=str,
        default='C:\\Users\\Kiet Le\\Downloads\\HEDGE\\apelid\\src\\apelid\\NSL_KDD\\resources\\NSLKDD\\clean_merged\\nslkdd_train_clean_merged.csv',
    ...
    parser.add_argument(
        ...
        type=str,
        nargs='+',
        default=['Benign', 'DoS', 'Probe', 'R2L', 'U2R'],
        help='All possible label values (default: NSL-KDD 5 classes)'
    )
    parser.add_argument(
        '--benign-label',
        type=str,
