"""
Cleanlab-based label noise defense for CIC-IDS2018.
...
from apelid.configs import CIC2018Resources
from apelid.preprocessing.cic2018_preprocessor import CIC2018Preprocessor
...
def resolve_repo_path(path_text: str) -> Path:
    ...
    candidates = [
        ...
        PROJECT_ROOT / path,
        PROJECT_ROOT / "src" / "apelid" / "IDS18" / path,
        PROJECT_ROOT / "src" / "apelid" / path,
    ]
...
def encode_dataset(df: pd.DataFrame, preprocessor: CIC2018Preprocessor, num_encoder: str) -> pd.DataFrame:
...
def inverse_transform_data(df_encoded: pd.DataFrame, preprocessor: CIC2018Preprocessor, num_encoder: str) -> pd.DataFrame:
...
def apply_cleanlab_filter(
    ...
    # Use model confidence on given label as quality score for all samples.
    row_idx = np.arange(len(y))
    label_quality_scores = pred_probs[row_idx, y_local]
    ...
    threshold_mask = label_quality_scores < float(threshold)
    ...
    return issues_idx, label_quality_scores, issue_info


def main() -> None:
    parser = argparse.ArgumentParser(description="Cleanlab-based label noise defense for CIC-IDS2018")
    parser.add_argument(
        "--input", "-i",
        type=str,
        default="src/apelid/IDS18/resources/IDS18/clean_merged/cic2018_train_original_undersampled_noise_30.csv"
    )
    ...
    # Recompute per-class stats properly
    per_class_removal = {}
