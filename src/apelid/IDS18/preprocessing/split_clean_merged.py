...
CLEAN_MERGED_DIR = apelid.configs.cic2018.CLEAN_MERGED_DATA_FOLDER
...
def _filter_files_by_labels(files: list[str], label_names: list[str] | None) -> list[str]:
    if not label_names:
        return files
    allowed_safe = {apelid.configs.cic2018.get_label_name(lbl) for lbl in label_names}
    out: list[str] = []
    for fp in files:
...
def main():
    ...
    if args.exclude_labels:
        excludes_safe = {apelid.configs.cic2018.get_label_name(lbl) for lbl in args.exclude_labels}
        kept: list[str] = []
