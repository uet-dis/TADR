"""
Auto ablation runner for CIC-IDS-2018 data-poisoning pipeline.
...
from apelid.configs.cic2018 import CIC2018Resources
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(description="Auto ablation pipeline runner for CIC-IDS-2018")
    parser.add_argument("--python-exe", type=str, default=sys.executable)
    parser.add_argument("--noise-rates", type=str, default="70",
                        help="Noise percentages for symmetric_label_noise, e.g. '10,20,30,40,50,60'")
    parser.add_argument("--kmeans-budget", type=int, default=6000)
    ...
    parser.add_argument("--clean-input", type=str, default=str(default_clean_input), help="Path to the clean CIC-IDS-2018 training CSV")
    ...
    for noise_pct in noise_rates:
        ...
        for row in run_training(python_exe, training_script, noise_output, phase_dirs["02_training_noise"], f"{run_tag}_01_after_noise", args.device):
        ...
        run_step(
            ...
            [
                ...
                "--device",
                "cuda" if args.device == "auto" else "cpu",
                "--log-level",
                "INFO",
                "--output-dir",
        ...
        for row in run_training(python_exe, training_script, dae_clean, phase_dirs["04_training_dae"], f"{run_tag}_02_after_dae", args.device):
            row["noise_pct"] = noise_pct
        ...
        for row in run_training(python_exe, training_script, recovered_clean, phase_dirs["06_training_recover"], f"{run_tag}_03_after_recover", args.device):
            row["noise_pct"] = noise_pct
