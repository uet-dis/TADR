"""
Cleanlab-based label noise defense for EdgeIIoTset.
...
def resolve_repo_path(path_text: str) -> Path:
    ...
    candidates = [
        ...
        SRC_ROOT / path,
        SRC_ROOT / "src" / "apelid" / "EdgeIIoTset" / path,
        SRC_ROOT / "src" / "apelid" / path,
    ]
...
def encode_dataset(df: pd.DataFrame, preprocessor: EDGEIOTPreprocessor, num_encoder: str) -> pd.DataFrame:
...
def inverse_transform_data(df_encoded: pd.DataFrame, preprocessor: EDGEIOTPreprocessor, num_encoder: str) -> pd.DataFrame:
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Cleanlab-based label noise defense for EdgeIIoTset")
    parser.add_argument(
        "--input", "-i",
        type=str,
        default="src/apelid/EdgeIIoTset/resources/edgeiot/clean_merged/edgeiot_train_clean_merged_noise_30.csv"
    )
    ...
    parser.add_argument("--only-benign", action="store_true", help="Only filter rows currently labeled as Normal")
