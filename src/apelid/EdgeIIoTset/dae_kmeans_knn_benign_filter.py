"""EdgeIIoT Benign defense: DAE embedding -> KMeans core -> KNN filtering.

Overall flow:
1) Read the poisoned input CSV.
2) Load the fitted EdgeIIoT encoders and encode the dataset.
3) Train a DAE and extract latent embeddings.
4) Keep only representative Normal core samples with KMeans.
5) Run a weighted KNN detector on the Normal core to flag suspicious rows.
6) Reconstruct the data back to the original feature space.
7) Save clean/noise CSVs and a JSON report.
...
DEFAULT_INPUT = Path(EDGEIOTResources.CLEAN_MERGED_FOLDER) / "edgeiot_train_clean_merged.csv"
...
def encode_dataset(df: pd.DataFrame, preprocessor: EDGEIOTPreprocessor) -> pd.DataFrame:
    """Select EdgeIIoT features and apply the fixed preprocessing pipeline."""
...
def inverse_transform_data(df_encoded: pd.DataFrame, preprocessor: EDGEIOTPreprocessor) -> pd.DataFrame:
    """Invert the encoded dataframe back to the original EdgeIIoT layout."""
    logger.info("[+] Inverse transforming dataset to original format...")
...
def select_benign_core_indices_from_embedding(
    ...
    y = df_embedded["Attack_type"].values
    ...
    logger.info(f"[+] Normal samples: {n_benign}")
...
class CorruptionDetector:
    ...
    def local_detection(
        ...
        if verbose:
            logger.info(f"[+] Normal candidates for KNN check: {n_eval} / {n_total}")
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="EdgeIIoT Benign defense: DAE embedding + KMeans core + KNN detection"
    )
    parser.add_argument("--input", "-i", type=str, default=str(DEFAULT_INPUT), help="Input poisoned CSV path")
    parser.add_argument("--output-dir", type=str, default=None, help="Output directory for all artifacts")
    ...
    logger.info(f"\n{'=' * 70}")
    logger.info("EdgeIIoT DEFENSE PIPELINE (DAE + KMEANS + KNN)")
    logger.info(f"{'=' * 70}")
    ...
    df_embedded["Attack_type"] = y_train
    ...
    # STEP 3: KMeans core (Normal only)
    logger.info(f"\n{'=' * 60}")
    logger.info("[+] STEP 3: KMEANS CORE (NORMAL ONLY)")
    logger.info(f"{'=' * 60}")
    ...
    logger.info(f"[+] Normal core kept: {len(core_set)}")
    logger.info(f"[+] Normal removed by KMeans: {len(kmeans_noise_set)}")

    # STEP 4: KNN filtering on the Normal-core subset
    logger.info(f"\n{'=' * 60}")
    logger.info("[+] STEP 4: KNN FILTER ON NORMAL-CORE")
    logger.info(f"{'=' * 60}")
    ...
    current_label_series = df_inverted["Attack_type"].astype(str)
    ...
    if original_label_series is not None:
        true_noise_mask = df_in["Attack_type"].astype(str).values != original_label_series.values
        ...
        logger.info(
            f"[+] Final Detection -> F1={metrics['f1']:.4f}, P={metrics['precision']:.4f}, "
    ...
    logger.info(f"\n{'=' * 70}")
    logger.info("EdgeIIoT DEFENSE PIPELINE COMPLETED")
    logger.info(f"{'=' * 70}")
